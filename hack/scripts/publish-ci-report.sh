...
# Use self-contained HTML reports for index.html (no Jekyll rendering needed).
# The HTML files are generated by k8s-dump-report.sh and k8s-run-scenario.py.
RUN_URL="${GITHUB_SERVER_URL}/${GITHUB_REPOSITORY}/actions/runs/${RUN_ID}"

if [ -f "${REPORT_PATH}/REPORT.html" ]; then
  cp "${REPORT_PATH}/REPORT.html" "${REPORT_PATH}/index.html"
  echo "Copied REPORT.html → index.html"
else
  # Fallback: generate a minimal index.html that links to REPORT.md.
  cat > "${REPORT_PATH}/index.html" <<FALLBACK_EOF
<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>${REPORT_TYPE} — PR #${PR_NUMBER}, Run #${RUN_NUMBER}</title></head>
<body><h1>${REPORT_TYPE} — PR #${PR_NUMBER}, Run #${RUN_NUMBER}</h1>
<p><a href="${RUN_URL}">View workflow run</a></p>
<p>HTML report not generated. <a href="./REPORT.md">View raw markdown report</a>.</p>
</body></html>
FALLBACK_EOF
  echo "REPORT.html not found; generated fallback index.html"
fi

# Copy per-scenario HTML reports as index.html.
if [ -d "${REPORT_PATH}/scenarios" ]; then
  for scenario_dir in "${REPORT_PATH}/scenarios"/*/; do
    [ -d "$scenario_dir" ] || continue
    scenario_name=$(basename "$scenario_dir")
    scenario_html="${scenario_dir}report.html"
    if [ -f "$scenario_html" ]; then
      cp "$scenario_html" "${scenario_dir}index.html"
      echo "Copied scenarios/${scenario_name}/report.html → index.html"
    fi
  done
fi
